...
const ScoreAssist = ({ submissionId, onPredictionLoaded }) => {
  ...
  const loadExistingPrediction = async () => {
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/ml/score-submission?submission_id=${submissionId}`, {
        headers: {
  ...
  const requestPrediction = async () => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/ml/score-submission`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
